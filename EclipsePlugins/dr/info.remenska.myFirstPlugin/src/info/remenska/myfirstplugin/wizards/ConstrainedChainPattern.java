...
public class ConstrainedChainPattern extends PropertyPattern {
	Model m;

	public ConstrainedChainPattern(Model m) {
		this.m = m;
	}
	...
	public void draw(int scope) {
		...
		case PropertyPattern.BEFORE:
			draw_beforeRConstrainedResponseChain(m);
			break;
		case PropertyPattern.AFTER:
			draw_afterQConstrainedResponseChain(m);
			break;
		case PropertyPattern.AFTER_LAST:
			draw_afterLastQConstrainedResponseChain(m);
			break;
		case PropertyPattern.BETWEEN:
			draw_betweenQandRConstrainedResponseChain(m);
			break;
		case PropertyPattern.UNTIL:
			draw_UntilRConstrainedResponseChain(m);
			break;
		case PropertyPattern.AFTER_UNTIL:
			draw_afterQUntilRConstrainedResponseChain(m);
			break;
...
public void draw_afterLastQConstrainedResponseChain(Model m2) {
	...
	Collaboration coll = (Collaboration) m.createPackagedElement("Collab_AfterLastQ_ConstrainedResponseChain_Pattern_" + random, UMLPackage.eINSTANCE.getCollaboration());
	Interaction inter = (Interaction) coll.createOwnedBehavior("Inter_AfterLastQ_ConstrainedResponseChain_Pattern_" +random, UMLPackage.eINSTANCE.getInteraction());
	afterLastQ(m, coll, inter);
	...
	// note slightly different syntax here
	Diagram d = UMLModeler.getUMLDiagramHelper().createDiagram(inter, UMLDiagramKind.SEQUENCE_LITERAL,inter);
	d.setName("Diag_SD_AfterLastQ_ConstrainedResponseChain_Pattern_"+random);		
	UMLModeler.getUMLDiagramHelper().openDiagramEditor(d);		

	Diagram cd = UMLModeler.getUMLDiagramHelper().createDiagram(inter, UMLDiagramKind.COMMUNICATION_LITERAL,inter);
	cd.setName("Diag_COM_AfterLastQ_ConstrainedResponseChain_Pattern_"+random);		
	}

public void draw_UntilRConstrainedResponseChain(Model m2) {
	int random = (int )(Math.random() * 5000 + 1);

	Collaboration coll = (Collaboration) m.createPackagedElement("Collab_UntilR_ConstrainedResponseChain_Pattern_" + random, UMLPackage.eINSTANCE.getCollaboration());
	Interaction inter = (Interaction) coll.createOwnedBehavior("Inter_UntilR_ConstrainedResponseChain_Pattern_" +random, UMLPackage.eINSTANCE.getInteraction());
	...
	// and finally... create the diagrams
			// note slightly different syntax here
	Diagram d = UMLModeler.getUMLDiagramHelper().createDiagram(inter, UMLDiagramKind.SEQUENCE_LITERAL,inter);
	d.setName("Diag_SD_UntilR_ConstrainedResponseChain_Pattern_"+random);		
	UMLModeler.getUMLDiagramHelper().openDiagramEditor(d);		
		
	Diagram cd = UMLModeler.getUMLDiagramHelper().createDiagram(inter, UMLDiagramKind.COMMUNICATION_LITERAL,inter);
	cd.setName("Diag_COMUntilR_ConstrainedResponseChain_Pattern_"+random);		
	
	}

public void draw_beforeRConstrainedResponseChain(Model m){
...
public void draw_afterQUntilRConstrainedResponseChain(Model m){
	...
	public void draw_afterQConstrainedResponseChain(Model m){
		int random = (int )(Math.random() * 5000 + 1);

		Collaboration coll = (Collaboration) m.createPackagedElement("Collab_AfterQ_ConstrainedResponseChain_Pattern_" + random, UMLPackage.eINSTANCE.getCollaboration());
		Interaction inter = (Interaction) coll.createOwnedBehavior("Inter_AfterQ_ConstrainedResponseChain_Pattern_" +random, UMLPackage.eINSTANCE.getInteraction());
		afterQ(m, coll, inter);
		...
		// note slightly different syntax here
		Diagram d = UMLModeler.getUMLDiagramHelper().createDiagram(inter, UMLDiagramKind.SEQUENCE_LITERAL,inter);
		d.setName("Diag_SD_AfterQ_ConstrainedResponseChain_Pattern_"+random);		
		UMLModeler.getUMLDiagramHelper().openDiagramEditor(d);		
	
		Diagram cd = UMLModeler.getUMLDiagramHelper().createDiagram(inter, UMLDiagramKind.COMMUNICATION_LITERAL,inter);
		cd.setName("Diag_COM_AfterQ_ConstrainedResponseChain_Pattern_"+random);		
	}
	

	public void draw_globallyConstrainedResponseChain(Model m){
		int random = (int )(Math.random() * 5000 + 1);

		Collaboration coll = (Collaboration) m.createPackagedElement("Collab_Globally_ConstrainedResponseChain_Pattern_" + random, UMLPackage.eINSTANCE.getCollaboration());
		Interaction inter = (Interaction) coll.createOwnedBehavior("Inter_Globally_ConstrainedResponseChain_Pattern_" +random, UMLPackage.eINSTANCE.getInteraction());
		...
		// note slightly different syntax here
		Diagram d = UMLModeler.getUMLDiagramHelper().createDiagram(inter, UMLDiagramKind.SEQUENCE_LITERAL,inter);
		d.setName("Diag_SD_Globally_ConstrainedResponseChain_Pattern_"+random);		
		UMLModeler.getUMLDiagramHelper().openDiagramEditor(d);		
	
		Diagram cd = UMLModeler.getUMLDiagramHelper().createDiagram(inter, UMLDiagramKind.COMMUNICATION_LITERAL,inter);
		cd.setName("Diag_COM_Globally_ConstrainedResponseChain_Pattern_"+random);
	}
	
	public void draw_betweenQandRConstrainedResponseChain(Model m){
		int random = (int )(Math.random() * 5000 + 1);

		Collaboration coll = (Collaboration) m.createPackagedElement("Collab_BetweenQandR_ConstrainedResponseChain_Pattern_" + random, UMLPackage.eINSTANCE.getCollaboration());
		Interaction inter = (Interaction) coll.createOwnedBehavior("Inter_BetweenQandR_ConstrainedResponseChain_Pattern_" +random, UMLPackage.eINSTANCE.getInteraction());
		afterQ(m, coll, inter);
		constrained_response_chain(m,coll, inter);
		beforeR(m,coll,inter);
		...
		// note slightly different syntax here
		Diagram d = UMLModeler.getUMLDiagramHelper().createDiagram(inter, UMLDiagramKind.SEQUENCE_LITERAL,inter);
		d.setName("Diag_SD__BetweenQandR_ConstrainedResponseChain_Pattern_"+random);		
		UMLModeler.getUMLDiagramHelper().openDiagramEditor(d);		
	
		Diagram cd = UMLModeler.getUMLDiagramHelper().createDiagram(inter, UMLDiagramKind.COMMUNICATION_LITERAL,inter);
		cd.setName("Diag_COM__BetweenQandR_ConstrainedResponseChain_Pattern_"+random);	
	}

public void constrained_response_chain(Model m, Collaboration coll, Interaction inter){
		
		
		regMessage(m, coll, inter, QuestionTreePage.textEventA);
		existence(m, coll, inter, QuestionTreePage.textEventB);
		constrained_absence(m, coll, inter, QuestionTreePage.textEventX);
		//// /// //// // // //// / ///
	...
	private void constrained_absence(Model m, Collaboration coll,
			Interaction inter, Text textEventX) {

		int random = (int )(Math.random() * 5000 + 1);

		Message originMessageA = QuestionTreePage.traceLineMap.get(textEventX).getOriginMessage();
		// we need the outter assert
		CombinedFragment assertFragment = (CombinedFragment) inter.getFragment("assertFragment");
		InteractionOperand assertOperand = assertFragment.getOperand("ASSERT_operand");
...
//		SendOperationEvent soe = (SendOperationEvent) m.createPackagedElement("SendOperationEvent1_"+random, UMLPackage.eINSTANCE.getSendOperationEvent());
		...
		CombinedFragment neg = (CombinedFragment) assertOperand.getFragment("NEG fragment", false, UMLPackage.eINSTANCE.getCombinedFragment(), true);
