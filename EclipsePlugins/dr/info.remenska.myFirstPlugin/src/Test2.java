...
//				expandItem1.setText(node.data);
//				expandItem1.setHeight(node.getLevel()*200);
//
//			} else{
//				Button label = new Button(composite1, SWT.RADIO);
//				label.setText(node.data);
//				label.addSelectionListener(selectionListener);
//			}
//			
//			System.out.println(ident+hmmm+node.data);
//			
...
//		ExpandItem expandItem1 = new ExpandItem(expandBar1, SWT.NONE, 0);
//		expandItem1
//				.setText(questionnaire.data);
//		expandItem1 
//				.setHeight(composite1.computeSize(SWT.DEFAULT, SWT.DEFAULT).y);
//		expandItem1.setControl(composite1);
		Stack<ExpandBar> expandBars = new Stack<ExpandBar>();
		Stack<ExpandItem> expandItems = new Stack<ExpandItem>();
		Stack<Composite> composites = new Stack<Composite>();
		for(TreeNode<String> node: treeRoot) {
			if (node.isQuestion){
				expandBars.add(new ExpandBar(shell, SWT.NONE));
				expandBars.peek().setLayoutData(new GridData(GridData.FILL, GridData.FILL,
						true, true, 2, 1));
				composites.add(new Composite(expandBars.peek(), SWT.NONE));
				GridLayout layout1 = new GridLayout(2, true);
				composites.peek().setLayout(layout1);
				expandItems.add(new ExpandItem(expandBars.peek(), SWT.NONE, 0));
				expandItems.peek().setText(node.data);
				expandItems.peek().setHeight(300+expandItems.size()*300);
				expandItems.peek().setControl(composites.peek());
			} else if (!node.isQuestion){
				Button label1 = new Button(composites.peek(), SWT.RADIO);
				label1.setText(node.data);
